...
Prefer heavy parallelism. Spawn as many subagents as the work can be cleanly
divided into, and run independent agents concurrently (launch them in a single
message with multiple Agent tool calls). More agents is better — but only when
they cannot step on each other.

Correctness matters far more than efficiency. Never trade a correct result for a
faster one. Before launching agents, partition the work so that no two agents
edit the same file or directory: give each agent an explicit, disjoint set of
files/paths to own, and keep shared or cross-cutting files (generated code,
`.proto` definitions, build config, `Cargo.toml`, etc.) under a single agent. If
the work cannot be cleanly partitioned, do it sequentially rather than risk a
conflict.

Multiple agents may be working simultaneously. If you see build errors in files
you did NOT edit, do not try to fix them. Wait 30 seconds and retry the build -
