...
<th class=optscol></th></tr>""".format(
    ...
    def _render_deck_node(self, node: DeckTreeNode, ctx: RenderDeckNodeContext) -> str:
        ...
        # next-review timer chip
        buf += "<td class=timertd>%s</td>" % self._deck_timer_chip(node)
        # per-deck action buttons
        buf += "<td class=actionstd>%s</td>" % self._deck_action_buttons(node)
        # options
        buf += (
    ...
    def _deck_seen_count(self, node: DeckTreeNode) -> bool:
        """Whether the deck has any already-seen (reviewed/learning) cards."""
        return bool(node.review_count or node.learn_count)

    def _next_due_seconds(self, did: DeckId) -> int | None:
        """Seconds until the soonest upcoming due card in this deck.

        Considers review cards (queue=2, ``due`` is a day number relative to
        ``col.sched.today``) and learning cards (queue=1, ``due`` is a unix
        timestamp). Only cards due strictly in the future are considered, so
        this is used when nothing is due right now. Deck-local for v1 (does
        not fold in subdeck cards). Returns None when nothing is upcoming.
        """
        col = self.mw.col
        db = col.db
        if db is None:
        ...
        next_rev_day = db.scalar(
            "select min(due) from cards where did=? and queue=2 and due>?",
            did,
            today,
        )
        if next_rev_day is not None:
            candidates.append((int(next_rev_day) - today) * 86400)

        next_lrn_ts = db.scalar(
            "select min(due) from cards where did=? and queue=1 and due>?",
            did,
            now,
        )
        if next_lrn_ts is not None:
            candidates.append(int(next_lrn_ts) - now)

        positive = [s for s in candidates if s > 0]
        if not positive:
            return None
        return min(positive)

    def _deck_timer_chip(self, node: DeckTreeNode) -> str:
        """A small chip showing when the next review of seen cards is due."""
        # Something is due right now (review or learning cards).
        if node.review_count or node.learn_count:
            return '<span class="db-chip db-chip-due">Due now</span>'

        secs = self._next_due_seconds(DeckId(node.deck_id))
        if secs is not None:
        ...
        # new/unseen cards), show a subtle marker; otherwise show nothing.
        if not self._deck_seen_count(node):
            return '<span class="db-chip db-chip-idle">Not started</span>'
        return ""

    def _deck_action_buttons(self, node: DeckTreeNode) -> str:
        """Continue (new cards remaining) and Review (review cards due now)."""
        buttons = ""
        # Review is shown only when review or learning cards are actually due.
        if node.review_count or node.learn_count:
            buttons += (
                "<button class='db-btn db-btn-review' "
                "onclick=\"event.stopPropagation(); return pycmd('review:%d')\">"
                "Review</button>" % node.deck_id
            )
        # Continue is shown when the deck still has new/unseen cards to learn.
        if node.new_count:
            buttons += (
                "<button class='db-btn db-btn-continue' "
                "onclick=\"event.stopPropagation(); return pycmd('continue:%d')\">"
                "Continue</button>" % node.deck_id
            )
        return buttons

    def _topLevelDragRow(self) -> str:
        return "<tr class='top-level-drag-row'><td colspan='11'>&nbsp;</td></tr>"
