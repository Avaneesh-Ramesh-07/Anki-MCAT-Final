...
def _confirm_import_over_studying_sample(mw: aqt.main.AnkiQt) -> bool:
    """Nudge the user to study the bundled sample deck before importing.

    Returns True only if the user explicitly chooses the understated
    "Import anyway" bypass. The prominent/default action steers them to study
    the sample deck first and cancels the import (returns False).
    """
    from aqt.utils import askUserDialog

    study_label = "Study the sample deck first"
    bypass_label = "Import anyway"
    diag = askUserDialog(
        "We recommend studying the built-in MCAT sample deck before importing"
        " your own decks. Give it a try first to get comfortable with how"
        " reviews and readiness tracking work.",
        [study_label, bypass_label],
        parent=mw,
        title="Import deck",
    )
    # make the "study first" choice the prominent, keyboard-default action
    diag.setDefault(0)
    # de-emphasize the bypass: give it the least-prominent role and shrink it so
    # it reads as a small, secondary escape hatch rather than a primary action.
    bypass_button = diag._buttons[1]
    if bypass_button is not None:
        bypass_button.setFlat(True)
        bypass_button.setStyleSheet("font-size: 10px; padding: 1px 4px;")
    choice = diag.run()
    return choice == bypass_label


def import_file(mw: aqt.main.AnkiQt, path: str) -> None:
    filename = os.path.basename(path).lower()

    # MCAT fork: gently steer users toward studying the bundled sample deck
    # before importing their own packaged decks. Only applies to user-initiated
    # deck packages (.apkg/.zip/.colpkg); the programmatic first-run seed uses
    # AnkiPackageImporter directly and never reaches this choke point.
    if filename.endswith((".apkg", ".zip", ".colpkg")):
        if not _confirm_import_over_studying_sample(mw):
            return
