...
class AnkiQt(QMainWindow):
    ...
    def setupProfile(self) -> None:
        # MCAT fork: remember whether this is the very first launch so we can
        # seed the bundled sample decks once the collection is loaded.
        self._seed_sample_decks_on_first_run = bool(self.pm.meta["firstRun"])
    ...
    def _bundled_sample_deck_paths(self) -> list[str]:
        """Locate the bundled MCAT sample decks under tools/.

        Tries several roots so this works both from a source checkout
        (qt/aqt/main.py -> repo root) and from a packaged/installed layout.
        """
        deck_names = ["mcat_sample.apkg", "mcat_sample_2.apkg"]
        candidate_roots: list[str] = []
        # source checkout: .../qt/aqt/main.py -> repo root is three levels up
        aqt_dir = os.path.dirname(os.path.abspath(__file__))
        candidate_roots.append(os.path.dirname(os.path.dirname(aqt_dir)))
        # packaged layouts / fallbacks
    ...
    def _seed_bundled_sample_decks(self) -> None:
        """Import the bundled sample decks once, on first run.

        Best-effort: any missing file or import error is logged and swallowed so
        it can never block startup. Uses AnkiPackageImporter directly (not
        import_file), so it deliberately bypasses the import-recommendation
        dialog.
        """
        if not self.col:
            return
        ...
        for path in self._bundled_sample_deck_paths():
            try:
                importer = AnkiPackageImporter(self.col, path)
                importer.run()
            except Exception:
                print(f"MCAT: failed to seed sample deck: {path}")
                traceback.print_exc()
    ...
    def loadProfile(self, onsuccess: Callable | None = None) -> None:
        ...
        # MCAT fork: on the very first launch, seed the bundled sample decks so
        # the home screen has content to study. Idempotent + best-effort: it only
        # runs once and never blocks startup.
        if getattr(self, "_seed_sample_decks_on_first_run", False):
            self._seed_sample_decks_on_first_run = False
            self._seed_bundled_sample_decks()
    ...
    def apply_collection_options(self) -> None:
        ...
        self._mcat_ensure_fsrs_enabled()

    def _mcat_ensure_fsrs_enabled(self) -> None:
        """MCAT: FSRS is always on in this fork.

        The memory model (the Mastery Query) reads FSRS DSR state, so a profile
        must never fall back to the legacy SM-2 scheduler. Enabling FSRS through
        the deck-config update path (rather than only flipping the config bool)
        also recomputes memory state for already-reviewed cards, so the readiness
        dashboard reflects prior study immediately. Idempotent: once FSRS is on,
        later loads are a no-op.
        """
        from anki.decks import UpdateDeckConfigs, UpdateDeckConfigsMode

        try:
            data = self.col.decks.get_deck_configs_for_update(DeckId(1))
            if data.fsrs:
                return
            selected_id = data.current_deck.config_id
            selected = next(
                c.config for c in data.all_config if c.config.id == selected_id
            )
            ...
            self.col.decks.update_deck_configs(req)
        except Exception:
            # Never block collection load if enabling FSRS fails.
            traceback.print_exc()
...
title="{}" {}>{}</button>""".format(
    ...
    def setupMenus(self) -> None:
        ...
        # MCAT readiness is surfaced on the home screen via the top toolbar
        # (Toolbar._readinessLinkHandler), not through the Tools menu.
