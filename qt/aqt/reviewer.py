...
"""
    ...
    def _shortcutKeys(
        ...
        return [
            ...
            *(
            ...
            ("1", partial(self._answerCard, 1)),  # MCAT: "Forgot" (Again)
            ("u", self.mw.undo),
    ...
    def _onAnswerButton(self, val: str) -> None:
        # MCAT: the default (spacebar) answer is "Got it", graded by time.
        self._answerCard(self._ease_from_time())
...
"""
    ...
    # MCAT: comfort is implicit — "Got it" is graded by how long the answer took
    # (Insight 3). Thresholds are v1 defaults on the (capped) answer time.
    GOTIT_EASY_MAX_MS = 5000  # <= 5s  -> Easy
    GOTIT_GOOD_MAX_MS = 15000  # <= 15s -> Good; slower -> Hard

    def _ease_from_time(self) -> Literal[2, 3, 4]:
        ms = self.card.time_taken(capped=True)
        if ms <= self.GOTIT_EASY_MAX_MS:
            return 4
        if ms <= self.GOTIT_GOOD_MAX_MS:
            return 3
        return 2
    ...
    def _answerButtons(self) -> str:
        # MCAT: two buttons only — "Forgot" (Again) and "Got it" (graded by time).
        def but(cmd: str, label: str, extra: str = "") -> str:
            return (
                "<td align=center><button %s onclick='pycmd(\"%s\");'>%s</button></td>"
                % (extra, cmd, label)
            )

        buf = "<center><table cellpadding=0 cellspacing=0><tr>"
        buf += but("forgot", "Forgot")
        buf += but("gotit", "Got it", 'id="defease"')
        buf += "</tr></table>"
        return buf
